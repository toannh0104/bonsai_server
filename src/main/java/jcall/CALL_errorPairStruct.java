...
public class CALL_errorPairStruct {
  static Logger logger = Logger.getLogger(CALL_errorPairStruct.class.getName());
  ...
  // Constructor
  public CALL_errorPairStruct(CALL_database data, String target, String observed, CALL_sentenceWordStruct wordT) {
    init();

    logger.debug("Enter CALL_errorPairStruct;");
    ...
    if (targetString == null) {
      logger.debug("targetString is null");
    }

    if (targetWord == null) {
      logger.info("targetWord is NULL");
    }

    if (observedString == null || observedString.length() == 0) {

      logger.debug("observedString is null, set observedWord");
  ...
  public void classifyError() {
    ...
    if ((targetWord != null) && (observedWord == null)) {
      ...
      logger.debug("subErrorType == DELETION");
    }
    if ((targetWord == null) && (observedWord != null)) {
      logger.error("(targetWord ==null) && (observedWord != null)");
      // INSERTIONS
    ...
    if ((targetWord != null) && (observedWord != null)) {
      ...
      logger.info("Two words are not null");
        ...
        if (targetComponentType != observedComponentType) {
          ...
          if (targetWord.useFormSettings) {
            ...
            if (targetComponentType == CALL_lexiconStruct.LEX_TYPE_PARTICLES) {
              ...
              // --------------------------------------------------
              if ((targetWord.word.kana.equals("も")) || (observedWord.word.kana.equals("も"))) {
                if ((targetWord.word.kana.equals("は")) || (observedWord.word.kana.equals("は"))
                    || (targetWord.word.kana.equals("が")) || (observedWord.word.kana.equals("が"))) {
    ...
    logger.debug("end of classifyError");
  }
  ...
  public boolean semanticMatch() {
    logger.debug("Enter semanticMatch");
  ...
  public double spellingErrorScore() {
    logger.debug("Enter spellingErrorScore");

    double rVal = 0.0;

    double distance = stringDistance();
    logger.debug("stringDistance(): " + distance);

    if ((!semanticMatch()) && (!stemMatch()) && (!typeMatch())) {
      logger.debug("It may be a known word, but if it is not seen to be matching in any other");
      // It may be a known word, but if it is not seen to be matching in any
      // other
      // significant way, we may do the spelling check
      if (distance <= SPELLING_DISTANCE_UPPER_THRESHOLD) {
        rVal = 0.5 + (((SPELLING_DISTANCE_UPPER_THRESHOLD - distance) / SPELLING_DISTANCE_UPPER_THRESHOLD) / 2.0);
      }
      logger.debug("rVal: " + rVal);
    } else {
      logger.debug("It may be a known word, and has some relation to the target word.");

      // It's a known word, and has some relation to the target word. But if
      // it's really close,
      // it may still be the result of a spelling error
      if (distance <= SPELLING_DISTANCE_LOWER_THRESHOLD) {
        rVal = 0.5 + (((SPELLING_DISTANCE_LOWER_THRESHOLD - distance) / SPELLING_DISTANCE_LOWER_THRESHOLD) / 2.0);
      }
      logger.debug("rVal: " + rVal);
    }
  ...
  public String errorTargetComponentString() {
    ...
    return CALL_lexiconStruct.typeInt2Name(componentType);

    // if((componentType >= 0) && (componentType <
    // CALL_lexiconStruct.typeString.length))
    // {
  ...
  public String errorObservedComponentString() {
    ...
    componentType = observedComponentType;
  ...
  public void logError() {
    ...
    // 2011.04.01 T.Tajima add
    // statement of errorType and subErrorType which through no substitution
    // path
    if ((errorType < 0) || (errorType >= typeStrings.length))
      errorType = 4;
    if ((subErrorType < 0) || (subErrorType >= typeStrings.length))
      subErrorType = 14;
    ...
    logger.debug("Error Type: " + typeStrings[errorType]);
    logger.debug("Sub-Error Type: " + subTypeStrings[subErrorType]);
    logger.debug("Probably Spelling Error: " + spellingErrorLikelihood);
