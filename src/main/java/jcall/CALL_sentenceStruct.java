...
public class CALL_sentenceStruct {
    static Logger logger = Logger.getLogger(CALL_sentenceStruct.class.getName());
    // Fields
    ...
    Vector<String> formNames; // The names of the form classes (verbForm,
                              // adjForm,
    ...
    // Variables for the form of the whole sentence (based on first specified
    // form
    // setting in the top level rule)
    // the form setting instance choosing from the first <form> , it values
    // equal
    ...
    public CALL_sentenceStruct() {
        init();
    }

    public CALL_sentenceStruct(CALL_database data, String gruleStr, CALL_conceptInstanceStruct instance) {
        init();
        db = data;
        createSentence(gruleStr, instance);
    }

    public CALL_sentenceStruct(CALL_database data, CALL_grammarRuleStruct grule,
            CALL_conceptInstanceStruct instance, FormSettingGrammarDto grammarDto)
    {
        init();
        db = data;
        createSentenceGHS(grule, instance, grammarDto);
    ...
    public void createSentence(String gruleStr, CALL_conceptInstanceStruct instance) {
        ...
        logger.debug("enter createSentence, based on concept instance: " + instance.toString());
    ...
    public void createSentenceGHS(CALL_grammarRuleStruct grule, CALL_conceptInstanceStruct instance, FormSettingGrammarDto grammarDto)
    {


            formSettingStructs = grammarDto.getFormSettingStructs();
            formSettings = grammarDto.getFormSettings();
            formNames = grammarDto.getFormNames();

            if (!formSet)
            {
                formSet = true;
                sign = grammarDto.getFormSettings().get(0).sign;
                tense = grammarDto.getFormSettings().get(0).tense;
                style = grammarDto.getFormSettings().get(0).style;
                type = grammarDto.getFormSettings().get(0).type;

            }
            // Also, if the sentence form is not
            // yet set, use this as the form
            recursive_resolve(null, null, grule, null, instance, -1, -1);
        ...
        if (wordID >= 0) {
            ...
            if (tempV != null) {
                ...
                for (int k = 0; k < tempV.size(); k++) {
                    // For each number word (tempV), and then each object word
                    // (tempV2),
                    // add a sentence word
    ...
    private Vector recursive_resolve(String fullRule, Vector prevWords, CALL_grammarRuleStruct grule, Vector parameters,
        ...
        logger.debug("Calling recursive resolve: Rule: " + grule.name + ", Cost: " + diffCost + ", Rule Path: " + fullRule);
        // CALL_debug.printlog(CALL_debug.MOD_SENTENCE, CALL_debug.DEBUG,
        // "Calling recursive resolve: Rule: " + grule.name + ", Cost: " +
        // diffCost
        // + ", Rule Path: " + fullRule);
        ...
        // STEP 1: PICK AND ADD FORMS SPECIFIED BY THIS RULE TO THE SENTENCE
        // RULE
        // SETTINGS
        // =====================================================================================
        ...
        for (int i = 0; i < grule.formSettings.size(); i++) {
            formVector = (Vector) grule.formSettings.elementAt(i);
            formName = (String) grule.formNames.elementAt(i);
            // Init Array form setting
            String[] formSetting = new String[10];
            if ((formVector != null) && (formName != null)) {
                // Now, pick one of the form templates from the form group at
                // random
                if (formVector.size() > 0) {
                    logger.debug("choose one setting in formVector, seletable size(): " + formVector.size());
                    // Get value first
                    form = (CALL_formStruct) formVector.elementAt(0);
                    // Add value of form in array
                    formSetting = form.toString().split(" ");
                    if (form != null) {
                        ...
                        formInstance = form.pickFirstSettings(db, instance, formSetting);
                        ...
                        if (formInstance != null) {
                            ...
                            logger.debug("Detemine form instance: " + formInstance.toString());

                            // Also, if the sentence form is not yet set, use
                            // this as the form
                            if (!formSet) {
                                formSet = true;
        ...
        // logger.debug("STEP 2: PARSE ALL OF THE TEMPLATES WITHIN THIS RULE");
        ...
        // Set the words added to the previous value, just in case it all
        // changes
        ...
        logger.debug("Templates size: " + grule.templates.size());

        for (int i = 0; i < grule.templates.size(); i++) {
            ...
            if (temp_template != null) {
                if (temp_template.restriction != null) {
                    ...
                    if (restriction_match(grule, temp_template.restriction, instance, parameters)) {
                        // CALL_debug.printlog(CALL_debug.MOD_SENTENCE,
                        // CALL_debug.DEBUG,
                        // "Adding restricted template: " +
                        // temp_template.structure);
                        logger.debug("Adding restricted template: " + temp_template.structure);
        ...
        // If we didn't match with any restricted templates, use the default
        // ones
        // ============================================================
        ...
        // Now go through all the valid templates at this level, expanding them,
        // and
        ...
        for (int t = 0; t < validTemplates.size(); t++) {
            ...
            logger.debug("Processing template[" + t + "]: " + template.toString());

            // Set the previous words vector (a list of all the words added in
            // the
            // last stage (final, appendable words)
            // We set this above, but it will likely have been changed during
            // the
            // processing of the last template
            previousWords = prevWords;

            if (template != null) {
                // Set the form for this template (if specified, else use the
                // default)
                currentTemplateForm = null; // CALL_formInstanceStruct
                                            // currentTemplateForm
                formStr = template.form; // form name, for example "basic"

                if (formStr != null) {
                    logger.debug("Search the form for this template, formStr: " + formStr);
                    ...
                    logger.debug("Form: " + currentTemplateForm.toString());
                    logger.debug("Form Struct(toString): " + currentTemplateFormStruct.toString());
                ...
                if (currentTemplateForm == null) {
                    ...
                    currentTemplateForm = new CALL_formInstanceStruct(0); // using
                                                                          // default
                                                                          // "pos present plain statement"
                ...
                while (st.hasMoreTokens()) {
                    ...
                    tempString = CALL_io.getNextBracketedToken(st); // This
                                                                    // should
                                                                    // thus
                                                                    // get {ITEM
                                                                    // cost} etc
                    ...
                    if (tempString != null) {
                        if (tempString.length() >= 3) {
                            ...
                            // If it's a variable, convert the variable to it's
                            // value
                            if (tempString.startsWith("|")) {
                                ...
                                // Should be 3 strings - the parameter, and the
                                // 2 costs
                                dataString = null;
                                ...
                                // CALL_debug.printlog(CALL_debug.MOD_SENTENCE,
                                // CALL_debug.DEBUG, "Variable - converting |" +
                                // dataString +
                                // "|, DCost: " + diffCostInt + ", CCost: " +
                                // comCostInt);
                                logger.debug("Variable - converting |" + dataString + "|, DCost: " + diffCostInt + ", CCost: "
                                        + comCostInt);

                                tempString = convertParameter(dataString, grule, parameters);
                                // CALL_debug.printlog(CALL_debug.MOD_SENTENCE,
                                // CALL_debug.DEBUG, "Converted to " +
                                // tempString);
                                logger.debug("Converted to " + tempString);

                                // We now use this string in the checks below to
                                // see what type
                                // it is...
                            } // end of variable
                            ...
                            if (tempString.equals("omit")) {
                                ...
                                // CALL_debug.printlog(CALL_debug.MOD_SENTENCE,
                                // CALL_debug.DEBUG, "Using string: " +
                                // keyString);
                                logger.debug("Using string: " + keyString);
                                // Should be 4 strings - the type, the value,
                                // and the 2 cost
                                // coefficients
                                typeString = null;
                                dataString = null;
                                ...
                                // CALL_debug.printlog(CALL_debug.MOD_SENTENCE,
                                // CALL_debug.DEBUG,
                                // "Adding lexical reference, type: " +
                                // typeString + ", Value: " + dataString +
                                // ", Cost: " +
                                ...
                                if ((typeString != null) && (dataString != null)) {
                                    ...
                                    if (wordType == CALL_lexiconStruct.LEX_TYPE_VERB || wordType == CALL_lexiconStruct.LEX_TYPE_ADJECTIVE
                                            || wordType == CALL_lexiconStruct.LEX_TYPE_ADJVERB) {
                                        if (dataString != null) {
                                            // We have a list of verbs, get the
                                            // right form, and add
                                            tempV = db.lexicon
                                            ...
                                            if (tempV != null) {
                                                ...
                                                for (int j = 0; j < tempV.size(); j++) {
                                                    ...
                                                    // The wordsAdded list is
                                                    // all words added at this
                                                    // template stage which will
                                                    // be
                                                    // follow on words in the
                                                    // next stage
                                                ...
                                                // CALL_debug.printlog(CALL_debug.MOD_SENTENCE,
                                                // CALL_debug.WARN,
                                                // "No words list found to match ["
                                                // +
                                                // dataString + "]");
                                            }
                                        }
                                    } else {
                                        // First, trim anything after and
                                        ...
                                        // CALL_debug.printlog(CALL_debug.MOD_SENTENCE,
                                        // CALL_debug.DEBUG, "Looking up " +
                                        // dataString);

                                        // Then add word Will need to actually
                                        // specifiy the type if
                                        // possible! - CJW
                                        tempV = db.lexicon.getWords(null, wordType, CALL_lexiconStruct.LEX_FORMAT_UNSPECIFIED, dataString);

                                        if (tempV != null) {
                                            // Add the words along with null
                                            // form structures (no form
                                            ...
                                            for (int j = 0; j < tempV.size(); j++) {
                                                ...
                                                if (wordType == CALL_lexiconStruct.LEX_TYPE_NOUN_SUFFIX) {
                                                    ...
                                                    // newWord.word.kanji +
                                                    // ", CostD: " +
                                                    // newWord.cost_grammar +
                                                    // ", CostC: " +
                                                    // newWord.cost_communication
                                                    // + ", Alt ID: " + j);

                                                    // Now add the word list to
                                                    // the sentence
                                                    if (words.size() > 0) {
                                                        // CALL_debug.printlog(CALL_debug.MOD_SENTENCE,
                                                        // CALL_debug.DEBUG,
                                ...
                                // There should be 2 strings, the reference, and
                                // the cost
                                // coefficient
                                dataString = null;
                                diffCostInt = diffCost;
                                ...
                                // CALL_debug.printlog(CALL_debug.MOD_SENTENCE,
                                // CALL_debug.DEBUG, "Conceptual reference: " +
                                // dataString +
                                // ", CostD: " + diffCostInt + ", CostC: " +
                                // comCostInt);
                                logger.debug("Conceptual reference: " + dataString + ", CostD: " + diffCostInt + ", CostC: "
                                        + comCostInt);

                                // Try and find this slot in the instance
                                logger.debug("Then, Try and find this slot in the instance");
                                for (int i = 0; i < instance.getLabel().size(); i++) {
                                    if (i >= instance.getData().size()) {
                                        // Must be a mismatch in the label and
                                        // data vectors
                                        break;
                                    }
                                    ...
                                    if (labelString != null) {
                                        if (labelString.equals(dataString)) {
                                            ...
                                            // CALL_debug.printlog(CALL_debug.MOD_SENTENCE,
                                            // CALL_debug.DEBUG,
                                            // "Found in instance!  Data: " +
                                            // dataString + ", ID: " + id);
                                            logger.debug("Found in instance!  Data: " + dataString + ", internal_id: " + id);

                                            if (id != null) {
                                                id_val = id.intValue();
                                                if (id_val >= 0) {
                                                    // Use the ID value to find
                                                    // the exact word we're
                                                    // after
                                                    tempV = db.lexicon.getWords(id_val); // id
                                                                                         // is
                                                                                         // the
                                                                                         // index
                                                                                         // of
                                            ...
                                            if ((tempV != null) && (tempV.size() > 0)) {
                                                // CALL_debug.printlog(CALL_debug.MOD_SENTENCE,
                                                // CALL_debug.DEBUG, "Found " +
                                                // tempV.size() +
                                                // " words in dictionary");

                                                logger.debug("Found " + tempV.size() + " words in dictionary");

                                                words = new Vector();

                                                // Now take the vector of words,
                                                // and combine with form
                                                // information
                                                for (int j = 0; j < tempV.size(); j++) {
                                                    // Get the correct form for
                                                    // each verb
                                                    ...
                                                    if (tempWord.type == CALL_lexiconStruct.LEX_TYPE_VERB
                                                        ...
                                                        // If its a verb (etc)
                                                        // need to do the form
                                                        logger.info("For VERBS, ADJECTIVES");
                                                        if (currentTemplateForm != null) {
                                                            logger.debug("new word form setting: " + currentTemplateForm.toString());
                                                            ...
                                                            if (currentTemplateFormStruct != null) {
                                                                newWord.formString = currentTemplateFormStruct.toString();
                                                                logger.debug("new word formString: " + currentTemplateFormStruct.toString());
                                                            } else {
                                                                logger.error("Word form is ok, but full formString is NULL");
                                                            }
                                                            ...
                                                            // + ", CostC: " +
                                                            // newWord.cost_communication);
                                                            logger.debug("Adding Verb/Adj/Adjverb: " + newWord.word.kanji + ", CostD: "
                                                                    + newWord.cost_grammar + ", CostC: " + newWord.cost_communication);
                                                        ...
                                                        if (template.counter_form != null) {
                                                            ...
                                                            if (id_val >= 0) {
                                                                // CALL_debug.printlog(CALL_debug.MOD_SENTENCE,
                                                                // CALL_debug.DEBUG,
                                                                // "Reference word(s) found");
                                                                logger.debug("Reference word(s) found, id_val: " + id_val);
                                                                // Use the ID
                                                                // value to find
                                                                // the exact
                                                                // word we're
                                                                ...
                                                                for (int k = 0; k < tempV2.size(); k++) {
                                                                    ...
                                                                    // CALL_debug.printlog(CALL_debug.MOD_SENTENCE,
                                                                    // CALL_debug.DEBUG,
                                                                    // "Adding sentence word: "
                                                                    // +
                                                                    // tempWord.kanji
                                                                    // +", counter reference "
                                                                    // +
                                                                    // tempWord2.kanji);
                                                                    logger.debug("Adding sentence word: " + tempWord.kanji + ", counter reference "
                                                                            + tempWord2.kanji);
                                                        ...
                                                        // + ", CostC: " +
                                                        // newWord.cost_communication);
                                                        logger.debug("Adding word: " + newWord.word.kanji + ", CostD: " + newWord.cost_grammar
                                                                + ", CostC: " + newWord.cost_communication);

                                                        words.addElement(newWord);

                                                        // The wordsAdded list
                                                        // is all words added at
                                                        // this
                                                        // template stage which
                                                        // will be
                                                        // follow on words in
                                                        // the next stage
                                                        wordsAdded.addElement(newWord);
                                                ...
                                                // Add the words to the
                                                // currently forming sentence
                                                // CALL_debug.printlog(CALL_debug.MOD_SENTENCE,
                                                // CALL_debug.DEBUG,
                                                // "Adding vector of " +
                                                // words.size()
                                                // + " words");
                                                logger.debug("Adding vector of " + words.size() + " words");

                                                addWordsToSentence(previousWords, words);
                                            } else {
                                                // No words - assume its a
                                                // disabled concept tag, don't
                                                // resolve the rule any further
                                                if (totalWordsAdded.size() <= 0) {
                                                    // No words added in this
                                                    // parse, use the words
                                                    // passed
                                                    // in originally
                                ...
                                if (!found) {
                                    // The concept does not exist (Changed from
                                    // returng
                                    // previousWords - CJW)
                                    return prevWords;
                                }
                                ...
                                // There should be 3 strings; the reference, and
                                // the two cost
                                // coefficients
                                dataString = null;
                                diffCostInt = diffCost;
                                ...
                                // Does the rule also have parameters (these are
                                // optional, so
                                ...
                                if (st2.hasMoreTokens()) {
                                    ...
                                    // CALL_debug.printlog(CALL_debug.MOD_SENTENCE,
                                    // CALL_debug.DEBUG, "Adding Subrule: " +
                                    // dataString +
                                    // "; CostD: " + diffCostInt + "; CostC: " +
                                    // comCostInt + ";"
                                    // + parmsToPass.size() + " Parameters");
                                    logger.debug("Adding Subrule: " + dataString + "; CostD: " + diffCostInt + "; CostC: " + comCostInt
                                            + ";" + parmsToPass.size() + " Parameters");
                                } else {
                                ...
                                if (subRule != null) {
                                    ...
                                    if (tempVector != null) {
                                        // Add the final words from the
                                        // sub-grammar into the added
                                        // words vector which
                                        // will get used as the previousWords in
                                        // the next template
                                        // item
                                        wordsAdded.addAll(tempVector);
                                        // for (int j = 0; j <
                                        // tempVector.size(); j++) {
                                        // CALL_sentenceWordStruct
                                        // }
                                    }
                                } else {
                                    ...
                                    // CALL_debug.printlog(CALL_debug.MOD_SENTENCE,
                                    // CALL_debug.ERROR,
                                    // "Failed to find subrule [" + keyString +
                                    // "]");
                    ...
                    // Now update the previous words vector to be all the
                    // (final) words
                ...
                // The last set of words added will be the final ones from the
                // template,
                        ...
                        if (tempString.length() >= 3) {
                            ...
                            if (tempString.startsWith("(")) {
                                ...
                                if ((typeString != null) && (dataString != null)) {
                                    ...
                                    // {
                                    // wordType =
                                    // CALL_lexiconStruct.LEX_TYPE_MISC;
                                    // }
                                    else if (typeString.equals("postfix")) {
                                        wordType = CALL_lexiconStruct.LEX_TYPE_NOUN_SUFFIX;
                                    } else if (typeString.equals("prefix")) {
    ...
    public boolean restriction_match(CALL_grammarRuleStruct grule, String restriction,
        ...
        logger.debug("Enter restriction_match");
            ...
            if (tempString != null) {
                if (tempString.startsWith("(")) // means have sub clauses
                {
                    ...
                    if (clause_level == 0) {
                        ...
                        // A sub-sub-clause. Add to string, will be dealt with
                        // in recursive
                        // function
                    ...
                    if (clause_level == 0) {
                        // Finished the sub-clause, process it and add to
                        // clauseString
        ...
        // Finished pre-processing the clause, now evaluate (should be 3 strings
        // for
        // comparrison clauses, or 2 in the case of "exists")
        logger
                .debug("Finished pre-processing the clause, now evaluate (should be 3 strings for comparrison clauses, or 2 in the case of [exists]");
                ...
                for (p = 0; p < prevWords.size(); p++) {
                    ...
                    if (prevWord != null) {
                        for (w = 0; w < words.size(); w++) {
                            word = (CALL_sentenceWordStruct) words.elementAt(w);
                            if (word != null) {
                                // Add word to previous word, linking back as
                                // well
    ...
    public Vector getWords() {
        ...
        if ((sentence != null) && (sentence.size() > 0)) {
            ...
            for (;;) {
                // Follow the nextWord links until reaching the end of the
                // sentence
    ...
    public Vector getSentenceWordVectors() {
        ...
        if ((sentence != null) && (sentence.size() > 0)) {
            ...
            for (;;) {
                ...
                // sentence
                if (nextSentenceWord != null) {
                    // What is the full grammar rule of this word? Add all the
                    // words in
    ...
    // =======================================================
    // add a filter between the right sentences;

    public Vector getSentenceWords() {
        logger.debug("enter getSentenceWords");

        Vector<CALL_sentenceWordStruct> rVector;
        CALL_sentenceWordStruct nextSentenceWord;

        rVector = new Vector<CALL_sentenceWordStruct>();

        if ((sentence != null) && (sentence.size() > 0)) {
            logger.debug("a vetor of sentenceWordStruct-->sentence, its size(): " + sentence.size());
    ...
    public String getSentenceString(int format) {
        logger.debug("enter getSentenceString, desired format: " + format);
        ...
        if (wordList != null) {
            logger.debug("worlList size: " + wordList.size());
            ...
            for (int i = 0; i < wordList.size(); i++) {
                ...
                if (tempSentenceWord != null) {
                    ...
                    if (tempWord != null) {
                        ...
                        // Get the formatted word string (including correct form
                        // etc)
                        tempString = tempSentenceWord.getTopWordString(format);

                        if (tempString != null) {
                            logger.debug("top word string: " + tempString);

                            if (tempSentenceWord.omit) {
                                logger.debug("Word omitted");
                                // Word omitted - need to consider sentence
                                // endings...
                            ...
                            logger.error("top word string: NULL");
                        }
                    }
                }
            }
        ...
        if ((parameters != null) && (grule.parameters != null)) {
            for (int p = 0; p < grule.parameters.size(); p++) {
                if (tempString.equals((String) grule.parameters.elementAt(p))) {
                    // We have found the matching parameter, now see if we have
                    // a value!
    ...
    private Vector recursiveStringConstructor(String prefix, CALL_sentenceWordStruct currentWord, Vector allStrings,
        ...
        if (currentWord != null) {
            ...
            // For counters...if a word can have more than one string, do that
            // here...
            alternatives = currentWord.getWordStrings(format);

            if (alternatives != null) {
                for (int a = 0; a < alternatives.size(); a++) {
                    // Note - we don't worry about word prefixes and suffixes
                    // here, as
                    // these are already contained within the alternatives
                    // vector
                    ...
                    if (wordString != null) {
                        ...
                        // Now gor through all the nextNode links, doing the
                        // recursive
                        // process
                        if (currentWord.nextWords.size() > 0) {
                            ...
                            // If this is the last word in the chain, add the
                            // final string to
    ...
    public CALL_formInstanceStruct getForm(String formStr) {
        logger.debug("Enter getForm, formStr is: " + formStr);
        ...
        for (int f = 0; f < formNames.size(); f++) {
            ...
            if (tempString.equals(formStr)) {
                form = (CALL_formInstanceStruct) formSettings.elementAt(f);// the
                                                                           ...
                                                                           // always
                                                                           // [0];
                logger.debug("Matched!" + " Matched index: " + f);
                break;
            }
        }
        return form;
    ...
    public CALL_formStruct getFormStruct(String formStr) {
        logger.debug("Enter getFormStruct, formStr is: " + formStr + " ");
        CALL_formStruct form = null;
        String tempString;
        ...
        for (int f = 0; f < formNames.size(); f++) {
            tempString = (String) formNames.elementAt(f);
            if (tempString.equals(formStr)) {
                form = (CALL_formStruct) formSettingStructs.elementAt(f);// the
                                                                         // matched
                                                                         // index
                                                                         // is
                                                                         // always
